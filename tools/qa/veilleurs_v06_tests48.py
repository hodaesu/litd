...
def main() -> int:
    ...
    test("25 canonical bridge markers", str(watchers_payload.get("schema_version", "")).startswith("0.6.2-canonical") and str(manifest.get("schema_version", "")).startswith("0.6.2-canonical") and source_load_ok)
    ...
    active_paths = [watchers_path, catalog_path, loadouts_path, ROOT / "scripts" / "core" / "content_db.gd", ROOT / "scripts" / "core" / "veilleurs_tactical_combat_runtime.gd"]
    ...
    runtime_paths = ["scripts/core/veilleurs_body_component.gd", "scripts/core/veilleurs_tactical_grid.gd", "scripts/core/veilleurs_tactical_combat_runtime.gd", "scripts/core/veilleurs_tactical_session.gd", "scripts/core/veilleurs_enemy_ai_v2.gd", "scripts/core/veilleurs_tactical_save_bridge.gd", "scripts/core/veilleurs_hybrid_generation_bridge.gd", "scripts/core/veilleurs_refuge_runtime.gd"]
    ...
    tiers_by_archetype = {archetype: {int(row.get("tier", 0)) for row in encounters if row.get("archetype") == archetype} for archetype in archetypes}
    test("45 exactly 64 encounters across eight 1-8 archetypes", encounters_payload.get("count") == 64 and len(encounters) == 64 and len(encounter_ids) == len(set(encounter_ids)) == 64 and len(archetypes) == 8 and all(tiers == set(range(1, 9)) for tiers in tiers_by_archetype.values()))
    ...
    for row in encounters:
        ...
        if not ids or not all(enemy_id in threat_by_enemy for enemy_id in ids): encounter_refs_valid = False; break
        computed = sum(threat_by_enemy[enemy_id] for enemy_id in ids)
        if abs(computed - float(row.get("threat_budget", computed))) > 0.75: encounter_refs_valid = False; break
        if not bool(row.get("retreat_viable", False)) or not str(row.get("counterplay", "")).strip(): encounter_refs_valid = False; break
        if row.get("archetype") == "mixed_memory" and not bool(row.get("memoriel_allowed", False)): encounter_refs_valid = False; break
    test("46 encounter references threat retreat and counterplay valid", encounter_refs_valid)

    playable_paths = ["scripts/ui/veilleurs_tactical_demo.gd", "scripts/ui/veilleurs_tactical_ui.gd", "scenes/veilleurs/v06_tactical_demo.tscn", "scenes/veilleurs/v06_tactical_combat.tscn", "scripts/core/veilleurs_tactical_save_bridge.gd"]
    ...
    if len(results) != 48: raise RuntimeError(f"Tests_48 contract drifted: {len(results)} checks")
