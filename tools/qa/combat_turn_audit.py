...
def run(root: Path = ROOT) -> dict:
    ...
    versions = {i: (root / f"scripts/ui/main_v{i}.gd").read_text(encoding="utf-8") for i in range(2, 15)}
    ...
    sources = [base] + [versions[i] for i in range(2, 15)]
    effective_combat = "\n".join(sources)
