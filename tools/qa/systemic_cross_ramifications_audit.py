...
EXPECTED_FAMILIES = {
    "food_and_logistics", "migration_and_refuge", "death_identity_and_memorial",
    "evidence_testimony_and_uncertainty", "nonhuman_consent",
    "scarce_resources_and_dragon_site", "relationships_and_bereavement",
}
EXPECTED_HEROES = {
...
ALLOWED_EXTERNAL = {
    ...
    "contextual_named_death", "winter_resource_pressure", "contradictory_late_evidence",
}
...
def audit_systemic_cross_ramifications(root: Path = ROOT) -> dict[str, Any]:
    ...
    for path, label in [
        (cross_path, "registre croisé"), (quests_path, "quêtes source"),
        (ramifications_path, "ramifications source"), (doc_path, "document narratif"),
    ]:
        if not path.is_file():
    ...
    source_pairs = {
        ...
        for q in quests.get("quests", []) for c in q.get("choices", [])
    }
    ram_pairs = {
    ...
    for key in [
        "crossings_use_existing_systems", "cross_event_requires_multiple_causes",
        "rumor_collision_preserves_sources", "contradiction_can_remain_unresolved",
        "hero_reactions_are_conditional", "deaths_remain_contextual",
        "relationship_effects_are_not_approval_scores",
    ]:
        if core.get(key) is not True:
            errors.append(f"Garde-fou {key} doit rester true")
    for key in [
        "new_universal_meter", "global_morality_score", "numeric_balance_values_locked_by_lore",
        "past_games_can_be_caused_by_litd1", "complete_seven_order_inferred",
    ]:
    ...
    for item in events:
        ...
        for raw in item.get("all_choices", []):
    ...
    if used_pairs != source_pairs:
        errors.append(f"Tous les 26 choix doivent participer à au moins un croisement; manquants={sorted(source_pairs-used_pairs)}")
    if hero_union != EXPECTED_HEROES:
        errors.append(f"Les Sept doivent tous pouvoir apparaître; manquants={sorted(EXPECTED_HEROES-hero_union)}")
    if family_seen != EXPECTED_FAMILIES:
        errors.append(f"Toutes les familles doivent être utilisées; manquantes={sorted(EXPECTED_FAMILIES-family_seen)}")
    if contextual_death_events < 6:
        errors.append("Plusieurs risques de mort contextuels doivent rester possibles")
    if len(rumor_modes_seen) < 5:
        errors.append("Les rumeurs doivent garder plusieurs niveaux de fiabilité")
    ...
    if "invasive_anchor_increase_on_saen" not in saen.get("forbidden_after_clear_refusal", []):
        errors.append("L'action invasive interdite sur Saen ne doit jamais être rouverte")
    ...
    if "miracle_does_not_prove_theology" not in set(lysandra.get("guardrails", [])):
        errors.append("Le garde-fou théologique de Lysandra doit rester actif")
    zeje = source_by_id.get("quest.litd1.zeje_three_versions_same_dead", {})
    if not {"unknown_is_valid", "probable_marker_must_be_visible"}.issubset(set(zeje.get("guardrails", []))):
        errors.append("PROBABLE et INCONNU doivent rester valides chez Zejé")
    ...
    if not required_effrie.issubset(set(effrie.get("guardrails", []))):
        errors.append("Les inconnues draconiques d'Èffrie doivent rester ouvertes")
    ...
    return {
        ...
        "warnings": warnings,
        "summary": {
            "source_choices": len(source_pairs), "cross_events": len(events),
            "families": len(family_seen), "cascades": len(cascades),
