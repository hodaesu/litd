#!/usr/bin/env python3
"""Detect moving-goalpost risk by crossing target history with real measurements."""
from __future__ import annotations

import argparse
import json
from hashlib import sha256
from pathlib import Path
from typing import Any

from tools.quality.target_drift_budget import relaxation_ratio

KIND = "LITD_GOALPOST_DRIFT_REPORT"


def _hash(payload: Any) -> str:
    return sha256(json.dumps(payload, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode()).hexdigest()


def _metric_series(rows: list[dict[str, Any]], target_id: str) -> list[float]:
    out: list[float] = []
    for row in rows:
        metrics = row.get("canonical_metrics", {})
        value = metrics.get(target_id) if isinstance(metrics, dict) else None
        if isinstance(value, (int, float)) and not isinstance(value, bool):
            out.append(float(value))
    return out


def _best_compatible_series(history: dict[str, Any], target_id: str):
    best: list[float] = []
    identity = None
    for group in history.get("groups", []):
        if not isinstance(group, dict):
            continue
        values = _metric_series(group.get("measurements", []), target_id)
        if len(values) > len(best):
            best = values
            identity = group.get("comparison_identity") if isinstance(group.get("comparison_identity"), dict) else None
    return best, identity


def _identity_key(identity: Any) -> str:
    return json.dumps(identity if isinstance(identity, dict) else {}, sort_keys=True, separators=(",", ":"), ensure_ascii=False)


def _bound_pair(binding: dict[str, Any], target_id: str, first_sequence: int, last_sequence: int):
    if binding.get("kind") != "LITD_MEASUREMENT_TARGET_BINDING" or binding.get("status") != "OK":
        return None, None, None, "invalid_or_missing_target_binding"
    buckets: dict[str, dict[str, Any]] = {}
    for row in binding.get("bindings", []):
        if not isinstance(row, dict) or row.get("binding_status") != "BOUND":
            continue
        metrics = row.get("canonical_metrics", {})
        value = metrics.get(target_id) if isinstance(metrics, dict) else None
        if not isinstance(value, (int, float)) or isinstance(value, bool):
            continue
        key = _identity_key(row.get("comparison_identity"))
        bucket = buckets.setdefault(key, {"identity": row.get("comparison_identity"), "first": [], "last": []})
        entry = (str(row.get("recorded_at", "")), float(value))
        if row.get("target_sequence") == first_sequence:
            bucket["first"].append(entry)
        if row.get("target_sequence") == last_sequence:
            bucket["last"].append(entry)
    usable = []
    for bucket in buckets.values():
        if bucket["first"] and bucket["last"]:
            bucket["first"].sort(key=lambda x: x[0])
            bucket["last"].sort(key=lambda x: x[0])
            usable.append(bucket)
    if not usable:
        return None, None, None, "insufficient_target_bound_measurements"
    if len(usable) > 1:
        return None, None, None, "ambiguous_compatible_measurement_groups"
    bucket = usable[0]
    start = bucket["first"][-1][1]
    end = bucket["last"][-1][1]
    return start, end, bucket["identity"], None


def _normalized_target(target: dict[str, Any]) -> dict[str, Any]:
    out = dict(target)
    if "type" not in out and "kind" in out:
        out["type"] = out["kind"]
    return out


def analyze(
    snapshots: list[dict[str, Any]],
    measurements: list[dict[str, Any]] | dict[str, Any],
    binding: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ordered = sorted(snapshots, key=lambda row: row.get("sequence", 0))
    if len(ordered) < 2:
        result = {
            "kind": KIND,
            "status": "INCONCLUSIVE",
            "reason": "insufficient_target_history",
            "signals": [],
            "core_write_allowed": False,
            "automatic_target_change_allowed": False,
        }
        result["report_hash"] = _hash(result)
        return result

    first, last = ordered[0], ordered[-1]
    signals: list[dict[str, Any]] = []
    reviewed = 0
    history_mode = isinstance(measurements, dict) and measurements.get("kind") == "LITD_MEASUREMENT_HISTORY"
    bound_mode = isinstance(binding, dict)

    for target_id in sorted(set(first.get("targets", {})) & set(last.get("targets", {}))):
        raw_base = first["targets"].get(target_id)
        raw_current = last["targets"].get(target_id)
        if not isinstance(raw_base, dict) or not isinstance(raw_current, dict):
            continue
        baseline_target = _normalized_target(raw_base)
        current_target = _normalized_target(raw_current)
        drift = relaxation_ratio(baseline_target, current_target)
        if drift.get("status") != "MORE_PERMISSIVE":
            continue
        reviewed += 1

        if bound_mode:
            start, end, identity, reason = _bound_pair(binding, target_id, int(first.get("sequence", 0)), int(last.get("sequence", 0)))
            if reason:
                signals.append({
                    "target_id": target_id,
                    "verdict": "INCONCLUSIVE",
                    "reason": reason,
                    "target_relaxation_ratio": drift.get("ratio"),
                    "comparison_identity": identity,
                    "target_binding_hash": binding.get("binding_hash"),
                })
                continue
            values = [start, end]
        elif history_mode:
            values, identity = _best_compatible_series(measurements, target_id)
        else:
            values = _metric_series(measurements if isinstance(measurements, list) else [], target_id)
            identity = None

        if len(values) < 2 or values[0] is None or values[-1] is None:
            signals.append({
                "target_id": target_id,
                "verdict": "INCONCLUSIVE",
                "reason": "insufficient_compatible_measurements",
                "target_relaxation_ratio": drift.get("ratio"),
                "measurement_count": len(values),
                "comparison_identity": identity,
            })
            continue

        start, end = float(values[0]), float(values[-1])
        kind = baseline_target.get("type")
        if kind == "max":
            improved = end < start
        elif kind == "min":
            improved = end > start
        elif kind == "window":
            low, high = float(baseline_target["min"]), float(baseline_target["max"])
            distance = lambda x: 0.0 if low <= x <= high else (low - x if x < low else x - high)
            improved = distance(end) < distance(start)
        else:
            signals.append({
                "target_id": target_id,
                "verdict": "INCONCLUSIVE",
                "reason": "unsupported_target_kind",
                "target_relaxation_ratio": drift.get("ratio"),
                "measurement_count": len(values),
                "comparison_identity": identity,
            })
            continue

        verdict = "NO_GOALPOST_SIGNAL" if improved else "GOALPOST_DRIFT_REVIEW_REQUIRED"
        signal = {
            "target_id": target_id,
            "verdict": verdict,
            "target_relaxation_ratio": drift.get("ratio"),
            "measurement_count": len(values),
            "first_measurement": start,
            "latest_measurement": end,
            "measurement_improved_against_original_target": improved,
            "comparison_identity": identity,
        }
        if bound_mode:
            signal.update({
                "first_target_sequence": first.get("sequence"),
                "latest_target_sequence": last.get("sequence"),
                "target_binding_hash": binding.get("binding_hash"),
            })
        signals.append(signal)

    flagged = sum(row["verdict"] == "GOALPOST_DRIFT_REVIEW_REQUIRED" for row in signals)
    result = {
        "kind": KIND,
        "status": "REVIEW_REQUIRED" if flagged else ("OK" if reviewed else "NO_PERMISSIVE_TARGET_DRIFT"),
        "summary": {"permissive_targets_reviewed": reviewed, "goalpost_signals": flagged},
        "signals": signals,
        "measurement_source": "target_bound_compatibility_safe_history" if bound_mode else ("compatibility_safe_history" if history_mode else "direct_measurements"),
        "authority": "advisory_only_core_or_human_decision_required",
        "core_write_allowed": False,
        "automatic_target_change_allowed": False,
    }
    result["report_hash"] = _hash(result)
    return result


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--snapshots", nargs="+", type=Path, required=True)
    parser.add_argument("--measurements", nargs="*", type=Path, default=[])
    parser.add_argument("--measurement-history", type=Path)
    parser.add_argument("--target-binding", type=Path)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    snapshots = [json.loads(path.read_text(encoding="utf-8")) for path in args.snapshots]
    if args.measurement_history:
        source = json.loads(args.measurement_history.read_text(encoding="utf-8"))
    else:
        source = []
        for path in args.measurements:
            payload = json.loads(path.read_text(encoding="utf-8"))
            if payload.get("kind") == "MEASUREMENT_CANDIDATE":
                source.extend(x for x in payload.get("reports", []) if isinstance(x, dict))
            else:
                source.append(payload)
    binding = json.loads(args.target_binding.read_text(encoding="utf-8")) if args.target_binding else None
    report = analyze(snapshots, source, binding)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(report, sort_keys=True, indent=2), encoding="utf-8")
    print(json.dumps({"status": report["status"], **report.get("summary", {})}, sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
