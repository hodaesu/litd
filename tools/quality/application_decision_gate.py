...
def _verify_evaluation_integrity(evaluation: dict[str, Any]) -> None:
    supplied = evaluation.get("evaluation_hash")
    if not _lower_hex(supplied, 64):
        raise ValueError("evaluation_hash must be 64 lowercase hex")
    canonical = dict(evaluation)
    canonical.pop("evaluation_hash", None)
    if _hash(canonical) != supplied:
        raise ValueError("bounded implementation evaluation integrity mismatch")
...
def evaluate(evaluation: dict[str, Any], decision: dict[str, Any]) -> dict[str, Any]:
    ...
    _verify_evaluation_integrity(evaluation)
