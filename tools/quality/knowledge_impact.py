...
def plan_impact(request: dict[str, Any], root: Path = REGISTRY_ROOT) -> ImpactPlan:
    ...
    registry = _load(root, "core_candidate_registry.json")
    matches = [item for item in registry["entries"] if item.get("id") == candidate_id]
    if matches:
        if matches[0] == candidate:
            return ImpactPlan("ALREADY_REGISTERED", "idempotent_existing_candidate", candidate_id, registry)
    ...
    return ImpactPlan("CORE_CHANGE_CANDIDATE", "active_knowledge_impact_ready_for_core_review", candidate_id, registry)


def apply_plan(plan: ImpactPlan, root: Path = REGISTRY_ROOT) -> None:
    if plan.status == "ALREADY_REGISTERED":
        return
    if plan.status != "CORE_CHANGE_CANDIDATE" or plan.registry_payload is None:
    ...
    try:
        ...
        os.replace(temp_name, root / "core_candidate_registry.json")
    finally:
