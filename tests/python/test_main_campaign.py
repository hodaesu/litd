...
def test_campaign_preserves_reveal_order():
    ...
    assert titles == ["Survivre aux Terres de Cendre", "Les traces d'avant la Chute", "Le Projet Seuil", "La Première Rupture", "Or-Silex et la Grande Fermeture", "Les Absents", "Les responsables vivants", "Le monde extérieur", "Ce qu'est réellement le Voile", "La lumière mérite d'être défendue"]
...
def test_final_orientations_are_unlocked_by_previous_world_state():
    ...
    assert set(endings) == {"radical_closure", "stable_coexistence", "preserve_crossings", "seek_absent", "restore_concord", "transform_concord"}
    for ending in endings.values():
        assert ending["requirements"] and ending["costs"]
        assert ending["best_form"].strip() and ending["dark_form"].strip()


def test_campaign_runtime_is_persistent_and_evaluates_endings():
    runtime = (ROOT / "scripts/core/campaign_state.gd").read_text()
    for contract in ("func complete_main_quest(quest_id: String)", "func active_main_quests()", "func available_endings()", "func ending_score_context()", "PoliticalState.three_awakenings", "func serialize()", "func deserialize(payload: Dictionary)"):
