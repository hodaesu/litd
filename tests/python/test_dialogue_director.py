...
def test_fourth_wall_lines_keep_dark_tone_and_distinct_voices() -> None:
    ...
    assert "nous appelons ça le destin" in joined
    assert "tous ceux qui étaient là au commencement" in joined
    assert "tu peux détourner les yeux. nous, non." in joined
    assert "celui qui décide" in joined
