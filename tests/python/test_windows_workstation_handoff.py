...
def test_setup_covers_required_workstation_software_and_backup():
    ...
    assert "GitHub.GitLFS" in setup
    assert "Microsoft.VisualStudio.2022.Community" in setup
    assert "EpicGames.EpicGamesLauncher" in setup
    assert "HeavyDataRoot" in setup
...
def test_preflight_records_pc_only_human_validation():
    ...
    assert "hardware_snapshot" in preflight
    assert "minimum-free-gb" in preflight
    assert '"unreal_editor"' in preflight
    assert "godot_43_compatible" in preflight
