...
def _is_integration_harness(path: Path) -> bool:
    """Integration smoke harnesses may exercise UI adapters by design."""
    return path.name.endswith("_smoke_test.gd") or path.name.endswith("_smoke_bootstrap.gd")


def test_production_core_does_not_depend_on_ui_layer() -> None:
    forbidden = (
        "res://scripts/ui/",
        '"scripts/ui/',
        "'scripts/ui/",
    ...
    for path in sorted(CORE_DIR.glob("*.gd")):
        if _is_integration_harness(path):
            continue
    ...
    assert not violations, (
        "production scripts/core must not depend directly on scripts/ui; "
