...
class PropAutomationTests(unittest.TestCase):
    def test_equipment_and_gameplay_props_are_covered(self):
        ...
        self.assertEqual(sum(job["category"] == "equipment" for job in jobs), 3)
...
class FullPipelineTests(unittest.TestCase):
    def test_pipeline_covers_all_production_domains(self):
        ...
        self.assertEqual(plan["summary"], {
            "total_jobs": 76, "materials": 1, "environments": 15, "characters": 52, "props": 8,
        })
        self.assertEqual(len(plan["stages"]), 76)
        self.assertEqual(len({job["job_id"] for job in plan["stages"]}), 76)
