...
def test_political_runtime_supports_automatic_unlocks_and_persistent_consequences():
    ...
    for contract in ("func refresh_unlocks()", "func _trigger_satisfied(trigger: Dictionary)", "GameState.expedition_room", "CreatureManager.captured_creatures.size()", "func completed_consequence(quest_id: String)", "func dialogue_context_for(npc_id: String)", "func service_unlocked(service_id: String)", "func serialize()", "func deserialize(payload: Dictionary)"):
        assert contract in runtime


def test_concord_ui_is_autoloaded_and_exposes_npcs_quests_and_social_state():
    project = (ROOT / "project.godot").read_text(); ui = (ROOT / "scripts/ui/political_ui.gd").read_text()
...
def test_decisions_change_material_and_civic_state_not_only_reputation():
    ...
    for quest in data["quests"]:
        for choice in quest["choices"].values():
            ...
            assert "trust" in effects and "tension" in effects and "three_awakenings" in effects
