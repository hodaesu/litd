...
# GE01 corpse interaction layer ported above current main_v48.
...
func _render_corpse_tactical_entry() -> void:
    ...
    button.name = "CorpseTacticsEntryGE01"
    button.position = Vector2(1038, 350)
    button.tooltip_text = "Utiliser les corps du champ de bataille : déplacer, barricader ou projeter."
    content.add_child(button)

func _render_corpse_action_menu() -> void:
    var panel := PanelContainer.new()
    panel.name = "CorpseTacticsMenuGE01"
...
func _render_corpse_choices(box: VBoxContainer) -> void:
    ...
    for index in range(corpse_ids.size()):
        ...
        row.add_child(make_button("CORPS %d · R%d" % [index + 1, slot], func(id = scar_id): corpse_selected_id = str(id); corpse_action_menu += "_rank"; show_screen("combat"), Vector2(150, 42)))
...
func _render_corpse_rank_choices(box: VBoxContainer) -> void:
    ...
    ranks.add_theme_constant_override("separation", 4)
    box.add_child(ranks)
    for rank in range(4):
...
func _execute_corpse_action(action: String, rank: int) -> void:
    ...
    if not bool(result.get("ok", false)):
        ...
        return
    if action == "push":
        GameState.add_log("Le corps est déplacé vers R%d. La ligne reste praticable." % (rank + 1))
    elif action == "barricade":
        GameState.add_log("Le corps barricade R%d : ligne bloquée · couverture 40 %% ." % (rank + 1))
    else:
        GameState.add_log("Le corps est projeté vers E%d : ligne ennemie bloquée%s." % [rank + 1, " · cible repoussée" if bool(result.get("displaced", false)) else ""])
