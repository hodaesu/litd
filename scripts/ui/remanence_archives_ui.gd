...
func _build() -> void:
    ...
    list = ItemList.new()
    list.custom_minimum_size = Vector2(405, 500)
...
func _refresh() -> void:
    ...
    list.clear()
    detail.text = "Sélectionne une entrée des Archives."
    summary.text = "%d adversaire(s) actifs · %d cicatrice(s) actives · %d événements conservés" % [
        RemanenceRuntime.entities.size(),
        RemanenceRuntime.world_scars.size(),
        RemanenceRuntime.event_timeline.size()
    ]
    match mode:
        "scars": _fill_scars()
...
func _fill_entities() -> void:
    ...
    for record: Dictionary in rows:
        ...
        var row := "%s%s · %s · score %d" % [prefix, str(record.get("name", "Adversaire")), _stage_name(str(record.get("stage", "normal"))), int(record.get("score", 0))]
...
func _fill_scars() -> void:
    ...
    for scar: Dictionary in rows:
        ...
        var row := "%s%s · %s · %s" % [prefix, str(scar.get("type", "trace")), str(scar.get("severity", "trace")), str(scar.get("age_stage", "fresh"))]
        var index := list.add_item(row)
        list.set_item_metadata(index, {"kind": "scar", "record": scar})

func _fill_timeline() -> void:
    for event: Dictionary in RemanenceRuntime.recent_events("", 80):
...
func _show_entity(metadata: Dictionary) -> void:
    ...
    lines.append("Statut : %s" % str(record.get("status", "inconnu")))
    ...
    if not adaptations.is_empty():
        lines.append("Adaptations : %s" % ", ".join(adaptations))
    lines.append("")
...
func _show_scar(scar: Dictionary) -> void:
    var lines: Array[String] = [
        "CICATRICE DU MONDE",
        "Type : %s" % str(scar.get("type", "trace")),
        ...
        "",
        str(scar.get("summary", "Aucun récit conservé."))
    ]
    detail.text = "\n".join(lines)
