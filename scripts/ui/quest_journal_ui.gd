...
func _render() -> void:
    ...
    if not CampaignState.discovered_revelations.is_empty():
        q.add_child(_label("RÉVÉLATIONS CONFIRMÉES",16,GOLD));
        for revelation in CampaignState.discovered_revelations.values(): q.add_child(_label("• %s" % String(revelation),13,MUTED))
