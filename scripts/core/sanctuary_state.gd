...
func _rule_matches(rule: Dictionary) -> bool:
    ...
    if PoliticalState.tension < int(rule.get("tension_min", -999)): return false
    if PoliticalState.trust < int(rule.get("trust_min", -999)): return false
    if PoliticalState.trust > int(rule.get("trust_max", 999)): return false
    if int(PoliticalState.three_awakenings.get("body", 0)) < int(rule.get("body_min", -999)): return false
    if int(PoliticalState.three_awakenings.get("city", 0)) < int(rule.get("city_min", -999)): return false
    if GameState.supplies > int(rule.get("supplies_max", 999)): return false
    var any_flags: Array = rule.get("any_flag", [])
    if not any_flags.is_empty():
        var found := false
        for flag_value in any_flags:
            if PoliticalState.is_flag_set(String(flag_value)): found = true; break
        if not found: return false
    var campaign_flags: Array = rule.get("campaign_any_flag", [])
    if not campaign_flags.is_empty():
        var campaign_found := false
        for flag_value in campaign_flags:
            if bool(CampaignState.chapter_flags.get(String(flag_value), false)): campaign_found = true; break
        if not campaign_found: return false
    return true
...
func refresh() -> void:
    ...
    for layer_value in data.get("layers", []):
        ...
        if layer_id == base_id: continue
        if _rule_matches(layer.get("when", {})): candidates.append(layer)
    ...
    for layer in candidates.slice(0, max_major): resolved.append(String(layer.get("id", "")))
...
func layer_definition(layer_id: String) -> Dictionary:
    for layer_value in data.get("layers", []):
        var layer: Dictionary = layer_value
        if String(layer.get("id", "")) == layer_id: return layer
    return {}
...
func gameplay_modifiers() -> Dictionary:
    ...
    for layer_id in active_layers:
        ...
        for key_value in modifiers.keys():
            var key := String(key_value); var value = modifiers[key]
            if typeof(value) == TYPE_BOOL: result[key] = bool(result.get(key, false)) or bool(value)
            else: result[key] = int(result.get(key, 0)) + int(value)
    return result
