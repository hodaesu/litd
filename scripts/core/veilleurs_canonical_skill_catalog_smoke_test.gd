...
func _ready() -> void:
    ...
    assert(str(report.get("decision_source", "")) == "canonical_prepc_pack_uncompressed_tree_json")
...
func _assert_canonical_action(action: Dictionary, all_runtime_ids: Dictionary, active_tree: String) -> void:
    ...
    assert(str(action.get("canonical_skill_source", "")) == "prepc_pack_uncompressed_tree_json")
