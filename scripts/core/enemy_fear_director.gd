...
func prepare_hero(hero: Dictionary) -> Dictionary:
    ...
    if not hero_records.has(hero_id):
        hero_records[hero_id] = {"dungeons_survived": 0, "bosses_defeated": 0, "enemies_defeated": 0, "critical_hits": 0, "ultimates_used": 0, "heroic_rescues": 0, "perfect_dungeons": 0, "deeds": {}, "family_victories": {}}
...
func party_threat(heroes: Array) -> float:
    ...
    for index in range(1, scores.size()): total += scores[index] * float(party_rule.get("other_heroes_weight", 0.35))
...
func initialize_enemy(enemy: Dictionary, heroes: Array) -> int:
    ...
    if bool(enemy.get("boss", false)): multiplier = float(courage.get("boss_multiplier", 0.55))
    elif bool(enemy.get("elite", false)): multiplier = float(courage.get("elite_multiplier", 0.75))
    ...
    enemy["enemy_fear"] = starting; enemy["enemy_fear_state"] = state_for(starting); enemy["enemy_fear_initial"] = starting
    return starting

func apply_event(enemy: Dictionary, event_id: String, context: Dictionary = {}) -> Dictionary:
    if enemy.is_empty() or int(enemy.get("hp", 0)) <= 0: return {}
    ...
    if bool(enemy.get("boss", false)) and delta > 0.0: delta *= float(data.get("courage", {}).get("boss_multiplier", 0.55))
...
func apply_witness_event(enemies: Array, event_id: String, context: Dictionary = {}) -> Array:
    ...
    for enemy_value: Variant in enemies:
        if enemy_value is Dictionary:
            ...
            if not result.is_empty(): results.append(result)
    return results
...
func change_fear(enemy: Dictionary, delta: int, reason: String) -> Dictionary:
    ...
    enemy["enemy_fear"] = after; enemy["enemy_fear_state"] = state_for(_effective_fear(enemy))
    if before != after: enemy_fear_changed.emit(enemy, before, after, reason)
...
func state_for(value: int) -> String:
    ...
    if fear >= int(thresholds.get("panic", 100)): return "panic"
    if fear >= int(thresholds.get("terrified", 75)): return "terrified"
    if fear >= int(thresholds.get("shaken", 50)): return "shaken"
    if fear >= int(thresholds.get("wary", 25)): return "wary"
    return "calm"

func combat_modifiers(enemy: Dictionary) -> Dictionary:
    if bool(enemy.get("ge01_fled", false)):
        return {"state": "fled", "accuracy_multiplier": -1.0, "damage_multiplier": 0.0, "retreat_bias": 1.0}
    var state := state_for(_effective_fear(enemy))
    if state == "calm": return {"state": state, "accuracy_multiplier": 1.0, "damage_multiplier": 1.0, "retreat_bias": 0.0}
    var values: Dictionary = data.get("gameplay", {}).get(state, {})
    var result := values.duplicate(true); result["state"] = state
    return result

func body_psychological_state(enemy: Dictionary) -> String:
    var state := state_for(_effective_fear(enemy))
    if state == "wary": return "tense"
    if state == "shaken" or state == "terrified": return "terrified"
    if state == "panic": return "panic"
    return "neutral"

func should_panic(enemy: Dictionary, round_number: int) -> bool:
    var effective := _effective_fear(enemy)
    if state_for(effective) != "panic": return false
...
func record_deed(hero: Dictionary, deed_id: String, amount: int = 1, family_id: String = "") -> void:
    var hero_id: String = str(hero.get("id", "")); var record: Dictionary = prepare_hero(hero)
    if record.is_empty(): return
    var field_map := {"dungeon_survived": "dungeons_survived", "boss_defeated": "bosses_defeated", "enemy_defeated": "enemies_defeated", "critical_hit": "critical_hits", "ultimate_used": "ultimates_used", "heroic_rescue": "heroic_rescues", "perfect_dungeon": "perfect_dungeons"}
    var field := str(field_map.get(deed_id, ""))
    if field != "": record[field] = maxi(0, int(record.get(field, 0)) + amount)
    var deeds: Dictionary = record.get("deeds", {}); deeds[deed_id] = maxi(0, int(deeds.get(deed_id, 0)) + amount); record["deeds"] = deeds
    if family_id != "":
        var families: Dictionary = record.get("family_victories", {}); families[family_id] = maxi(0, int(families.get(family_id, 0)) + amount); record["family_victories"] = families
    hero_records[hero_id] = record; hero_renown_changed.emit(hero_id, renown_score(hero))

func serialize() -> Dictionary: return {"hero_records": hero_records.duplicate(true)}
func deserialize(payload: Dictionary) -> void: hero_records = payload.get("hero_records", {}).duplicate(true)
