...
func load_game() -> bool:
    ...
    for hero_value in GameState.party: HeroSkillManager.prepare_hero(hero_value); EnemyFearDirector.prepare_hero(hero_value)
    EquipmentManager.deserialize(payload.get("equipment",{})); CreatureManager.deserialize(payload.get("creatures",{})); GameState.expedition_room = int(payload.get("expedition_room",0)); PoliticalState.deserialize(payload.get("politics",{})); CampaignState.deserialize(payload.get("campaign",{})); EnemyFearDirector.deserialize(payload.get("hero_renown",{})); AshlandsRuntime.deserialize(payload.get("ashlands",{}))
