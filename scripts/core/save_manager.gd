...
const VEILLEURS_RUNTIME := preload("res://scripts/core/veilleurs_runtime_facade.gd")
...
func save_game(slot: int = active_slot) -> bool:
    ...
    if success and slot == 0:
        success = _atomic_write(SAVE_PATH, body)
    if success:
...
func _build_payload() -> Dictionary:
    return {
        ...
        "metadata": {
            ...
            "mode": "veilleurs" if VEILLEURS_RUNTIME.is_active() else "litd1",
        ...
        "veilleurs": VEILLEURS_RUNTIME.serialize(),
...
func _apply_payload(payload: Dictionary) -> void:
    ...
    VEILLEURS_RUNTIME.deserialize(payload.get("veilleurs",{}))
...
func _migrate(payload: Dictionary) -> Dictionary:
    ...
    if not payload.has("veilleurs"):
        payload["veilleurs"] = payload.get("veilleurs_vs001",{})
