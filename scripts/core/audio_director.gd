...
func _ensure_music_player() -> void:
    ...
    _music_player.name = "AdaptiveMusicPlayer"
...
func _request_music(cue_id: String, context: Dictionary) -> Dictionary:
    ...
    payload["has_verified_local_audio"] = _try_play_music_candidate(active_music_candidate)
    cue_requested.emit("music", cue_id, payload)
    return payload

func _try_play_music_candidate(candidate: Dictionary) -> bool:
...
func _request_sfx(cue_id: String, context: Dictionary) -> Dictionary:
    ...
    payload["has_verified_local_audio"] = false
