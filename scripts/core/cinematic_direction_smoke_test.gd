...
func run() -> void:
    ...
    assert(not runtime.physical_profile("mathilde").is_empty(), "Mathilde physical profile missing")
    assert(not runtime.physical_profile("hungry_ghoul").is_empty(), "Hungry Ghoul physical profile missing")
    assert(not runtime.proxemic_pair("mathilde:aurelien").is_empty(), "core proxemic pair missing")
    ...
    assert(not runtime.dialogue_scene("demo_mathilde_ghoul_01").is_empty(), "dialogue must resolve to staged scene")
