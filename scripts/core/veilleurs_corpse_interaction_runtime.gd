...
func _study_aurelien(scar_id: String) -> Dictionary:
    ...
    RemanenceRuntime.link_archive_nodes("hero:aurelien", scar_id, "studied_corpse", {"run_index": RemanenceRuntime.run_index})
    var origin_entity_id := str(scar.get("origin_entity_id", ""))
    if origin_entity_id != "":
        RemanenceRuntime.link_archive_nodes("hero:aurelien", origin_entity_id, "anatomy_learned_from_corpse", {"scar_id": scar_id})
...
func _description(scar: Dictionary, payload: Dictionary) -> String:
    ...
    elif bool(payload.get("studied_by_aisha", false)):
        parts.append("Une analyse anatomique historique est déjà inscrite sur ce corps.")
    if bool(payload.get("prepared_as_cover", false)):
        parts.append("Le corps a été préparé comme couverture tactique.")
    if int(payload.get("postmortem_mutilation_count", 0)) > 0:
