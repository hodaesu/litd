...
func start_run(seed_value: int) -> Dictionary:
    var run_seed: int = seed_value if seed_value != 0 else int(Time.get_unix_time_from_system())
    var dungeon: Array = generate_dungeon(run_seed)
    active_run = {
        "seed": run_seed,
        "active": true,
        "dungeon": dungeon,
    ...
    run_started.emit(run_seed, dungeon.duplicate(true))
...
func generate_dungeon(seed_value: int) -> Array:
    ...
    var depth_count: int = rng.randi_range(int(depth_rules.get("min", 3)), int(depth_rules.get("max", 5)))
    var dungeon: Array = []
    var previous_room_id: String = ""
    for depth in range(depth_count):
        var room_count: int = rng.randi_range(int(depth_rules.get("rooms_per_depth_min", 4)), int(depth_rules.get("rooms_per_depth_max", 6)))
        for room_index in range(room_count):
            var room_type: String = _pick_room_type(rng)
            if depth == 0 and room_index == 0:
                room_type = "start"
            elif depth == depth_count - 1 and room_index == room_count - 1:
                room_type = "boss"
            var room_id: String = "d%02d_r%02d" % [depth + 1, room_index + 1]
            var hazards: Array = []
    ...
    for index in range(1, dungeon.size() - 2):
        if rng.randf() < 0.18:
            var target_index: int = mini(dungeon.size() - 1, index + 2)
            var current_room: Dictionary = dungeon[index]
            var target_room: Dictionary = dungeon[target_index]
            var target_id: String = str(target_room.get("id", ""))
            if not current_room["connections"].has(target_id):
                current_room["connections"].append(target_id)
                target_room["connections"].append(str(current_room.get("id", "")))
    return dungeon
...
func enter_room(room_id: String) -> Dictionary:
    ...
    room["visited"] = true
    var risk: Dictionary = current_risk_profile(int(room.get("depth", 1)))
...
func _consume_light_for_room() -> void:
    var decay: int = int(rules.get("light", {}).get("room_decay", 1))
    var current: int = int(ExpeditionManager.inventory.get("light", 0))
...
func dim_light(amount: int = 1) -> Dictionary:
    ...
    ExpeditionManager.inventory_changed.emit(ExpeditionManager.inventory.duplicate(true))
    var risk: Dictionary = current_risk_profile()
...
func current_risk_profile(depth_override: int = -1) -> Dictionary:
    ...
    var depth_rules: Dictionary = rules.get("depth", {})
    var max_light: int = maxi(1, int(light_rules.get("max", 10)))
    var light: int = clampi(int(ExpeditionManager.inventory.get("light", 0)), 0, max_light)
    var darkness: float = 1.0 - (float(light) / float(max_light))
    var depth: int = depth_override
    if depth < 0:
        depth = maxi(1, int(active_run.get("deepest_depth", 1)))
    var danger: float = 1.0 + darkness * float(light_rules.get("dark_danger_bonus", 0.9)) + float(depth - 1) * float(depth_rules.get("danger_bonus_per_depth", 0.04))
    var loot: float = 1.0 + darkness * float(light_rules.get("dark_loot_bonus", 0.75)) + float(depth - 1) * float(depth_rules.get("loot_bonus_per_depth", 0.03))
    var essence: float = 1.0 + darkness * float(light_rules.get("dark_essence_bonus", 0.6))
...
func generate_loot(depth: int, source: String = "room", seed_salt: int = 0) -> Dictionary:
    var rng: RandomNumberGenerator = RandomNumberGenerator.new()
    rng.seed = int(active_run.get("seed", 1)) + depth * 7919 + seed_salt * 104729 + int(source.hash())
    var profile: Dictionary = current_risk_profile(depth)
    var rarity: String = _pick_rarity(rng, float(profile.get("loot_multiplier", 1.0)))
    var affix_count: int = int(rules.get("affix_counts", {}).get(rarity, 1))
    var selected: Array = _pick_affixes(rng, affix_count)
    return {
        "id": "loot_%s_%s_%d" % [rarity, source, absi(rng.randi())],
        "seed": rng.seed,
        "rarity": rarity,
        "source": source,
        "depth": depth,
...
func _pick_affixes(rng: RandomNumberGenerator, count: int) -> Array:
    var pool: Array = rules.get("affixes", []).duplicate()
    var selected: Array = []
...
func _pick_rarity(rng: RandomNumberGenerator, loot_multiplier: float) -> String:
    ...
    var ordered: Array[String] = ["common", "uncommon", "rare", "epic", "legendary"]
    var adjusted: Array[float] = []
    var total: float = 0.0
    for index in range(ordered.size()):
        var rarity: String = ordered[index]
        var weight: float = float(weights.get(rarity, 0))
        if index >= 2:
...
func can_capture(enemy: Dictionary, zone_id: String) -> Dictionary:
    ...
    if bool(enemy.get("boss", false)) or bool(enemy.get("is_boss", false)) or enemy_id.to_lower().contains("angel") or enemy_name.to_lower().contains("ange"):
    ...
    var max_hp: int = maxi(1, int(enemy.get("max_hp", enemy.get("hp_max", 1))))
    var hp_ratio: float = float(int(enemy.get("hp", 0))) / float(max_hp)
...
func register_capture(enemy: Dictionary, zone_id: String) -> Dictionary:
    var check: Dictionary = can_capture(enemy, zone_id)
...
func commit_party_deaths(cause: String = "expedition") -> Array:
    ...
    for index in range(GameState.party.size() - 1, -1, -1):
        ...
        if int(hero.get("hp", 0)) > 0:
            continue
        var hero_id: String = str(hero.get("id", "hero_%d" % index))
...
func record_enemy_knowledge(enemy_id: String, killed: bool = false) -> Dictionary:
    ...
    var score: int = maxi(int(entry["encounters"]), int(entry["kills"]))
...
func finish_run(reason: String) -> Dictionary:
    ...
    var summary: Dictionary = extraction_summary()
