...
func _ready() -> void:
    var director := DIRECTOR_SCRIPT.new() as VeilleursCanonicalEncounterDirector
    director.reload_content()

    var report: Dictionary = director.validation_report()
    ...
    for source: Dictionary in director.encounter_records:
        var runtime_entry: Dictionary = director.runtime_for_named_encounter(str(source.get("name", "")))
    ...
    var charognards: Dictionary = director.runtime_for_named_encounter("Charognards du bord")
    ...
    var first: Dictionary = director.select_encounter(4242, "I")
    ...
    var second: Dictionary = director.select_encounter(4242, "I")
    ...
    var synergy_encounter: Dictionary = director.runtime_for_named_encounter("Faim derrière la masse")
    ...
    var memorial_encounter: Dictionary = director.runtime_for_named_encounter("Gardien isolé", {"memorial_candidate": memorial})
    ...
    var nemesis_encounter: Dictionary = director.runtime_for_named_encounter("Gardien isolé", {"memorial_candidate": artificial_nemesis})
    ...
    var lived_nemesis: Dictionary = director.runtime_for_named_encounter("Gardien isolé", {"memorial_candidate": artificial_nemesis})
    assert(bool((lived_nemesis.get("memorial_overlay", {}) as Dictionary).get("insert", false)))

    var full_encounter: Dictionary = director.runtime_for_named_encounter("Avant l’Orateur", {"memorial_candidate": memorial})
    ...
    var restored := DIRECTOR_SCRIPT.new() as VeilleursCanonicalEncounterDirector
    restored.reload_content()
    ...
    var deterministic_a := DIRECTOR_SCRIPT.new() as VeilleursCanonicalEncounterDirector
    var deterministic_b := DIRECTOR_SCRIPT.new() as VeilleursCanonicalEncounterDirector
    deterministic_a.reload_content()
    deterministic_b.reload_content()
    var choice_a: Dictionary = deterministic_a.select_encounter(777, "III")
    var choice_b: Dictionary = deterministic_b.select_encounter(777, "III")
