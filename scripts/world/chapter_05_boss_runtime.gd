...
func refresh() -> void:
    ...
    var hp := int(boss.get("hp",0)); var max_hp := maxi(1,int(boss.get("max_hp",1)))
    if last_hp >= 0 and hp < last_hp:
        repeated_damage_turns += 1
    else:
        repeated_damage_turns = maxi(0,repeated_damage_turns - 1)
    last_hp = hp
    if repeated_damage_turns >= 3:
