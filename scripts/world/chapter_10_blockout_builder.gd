extends Node3D
class_name Chapter10BlockoutBuilder
...
const STAKE_SCRIPT := preload("res://scripts/world/chapter_10_stake.gd")
const NODE_SCRIPT := preload("res://scripts/world/chapter_10_node.gd")

@export var zone_id := "c10_world_council"
...
func build_zone() -> void:
    world = _load_json(WORLD_PATH); zone = _find_zone(zone_id)
    if zone.is_empty(): push_error("Chapter10BlockoutBuilder: zone inconnue %s" % zone_id); return
    AshlandsRuntime.enter_zone(zone_id)
    _build_floor(); _build_boundaries(); _build_exits(); _build_encounters(); _build_stakes(); _build_nodes(); _build_campfire()
...
func _root() -> Node3D:
    var root := get_node_or_null("GeneratedChapter10") as Node3D
    if root == null: root = Node3D.new(); root.name = "GeneratedChapter10"; add_child(root)
    return root

func _build_floor() -> void:
    var size: Array = zone.get("size_m", [100,80]); var body := StaticBody3D.new(); _root().add_child(body)
...
func _build_exits() -> void:
    for value in zone.get("exits", []):
        var data: Dictionary = value; var gate := ZoneTransitionGate.new(); gate.from_zone = zone_id; gate.to_zone = String(data.get("to", "")); gate.gate_id = "%s_to_%s" % [zone_id, gate.to_zone]; gate.position = _vec3(data.get("position", [0,0,0])); _area_box(gate,Vector3(3,2.5,3)); _root().add_child(gate)

func _build_encounters() -> void:
    for value in zone.get("encounters", []):
        var data: Dictionary = value; var trigger := EncounterTrigger.new(); trigger.encounter_id = String(data.get("id", "")); trigger.encounter_type = String(data.get("type", "normal")); trigger.position = _vec3(data.get("position", [0,0,0]))
...
func _build_stakes() -> void:
    var ids: Array = zone.get("stakes", [])
    for i in ids.size():
        var item := STAKE_SCRIPT.new() as Chapter10Stake; item.name = String(ids[i]); item.position = Vector3(-18.0 + float(i) * 18.0,0,-10.0 + float(i % 2) * 20.0); item.configure(String(ids[i])); _area_box(item,Vector3(1.8,1.8,1.8)); _root().add_child(item)

func _build_nodes() -> void:
    for id_value in zone.get("nodes", []):
        var node_id := String(id_value); var data := _node_data(node_id)
        if data.is_empty(): continue
        var item := NODE_SCRIPT.new() as Chapter10Node; item.name = node_id; item.position = _vec3(data.get("position", [0,0,0])); item.configure(node_id); _area_box(item,Vector3(2.2,2.2,2.2)); _root().add_child(item)
...
func _build_player() -> void:
    var party := PARTY_SCENE.instantiate(); party.name = "Chapter10PartyRuntime"; party.position = _vec3(zone.get("entry", [0,0,0])) + Vector3.UP * 0.1; _root().add_child(party)
