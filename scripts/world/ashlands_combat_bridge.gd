    ...
    for enemy_id in ids:
        ...
        if encounter_type == "miniboss" and not miniboss_data.is_empty():
            ...
            if encounter_id == "c02_broken_curator":
                e["hp"] = 82; e["max_hp"] = 82; e["damage"] = [6,11]; e["fear"] = 6; e["chapter_miniboss_id"] = encounter_id
        ...
        if encounter_type == "boss":
            ...
            if encounter_id == "c01_boss_ash_witness":
                e["name"] = "Le Témoin des Cendres"; e["hp"] = 110; e["max_hp"] = 110; e["damage"] = [7,12]; e["fear"] = 7; e["chapter_boss_id"] = encounter_id; e["signature"] = "Dernier Souvenir du Jour"
            elif encounter_id == "c02_marker_warden":
                e["name"] = "Sahra Vel — La Veilleuse des Bornes"; e["hp"] = 128; e["max_hp"] = 128; e["damage"] = [7,12]; e["fear"] = 6; e["chapter_boss_id"] = encounter_id; e["signature"] = "La Carte qui se Souvient"
...
func _apply_loot(loot: Dictionary) -> void:
    ...
    var possible: Array = loot.get("possible",[]); var rolls := int(loot.get("rolls",0))
    if not possible.is_empty() and rolls > 0:
        var rng := RandomNumberGenerator.new(); rng.seed = ExpeditionManager.expedition_seed + encounter_id.hash()
        for _i in rolls: ExpeditionManager.add_resource(str(possible[rng.randi_range(0,possible.size()-1)]),1)
