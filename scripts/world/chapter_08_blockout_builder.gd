extends Node3D
class_name Chapter08BlockoutBuilder
...
func _root() -> Node3D:
    var root := get_node_or_null("GeneratedChapter08") as Node3D
    if root == null:
        root = Node3D.new(); root.name = "GeneratedChapter08"; add_child(root)
    return root
...
func _build_records() -> void:
    var ids: Array = zone.get("records", [])
    for i in ids.size():
        var entry := RECORD_SCRIPT.new() as Chapter08Record
        entry.name = String(ids[i]); entry.position = Vector3(-18.0 + float(i) * 18.0,0,-8.0 + float(i % 2) * 16.0); entry.configure(String(ids[i])); _area_box(entry,Vector3(1.6,1.6,1.6)); _root().add_child(entry)
...
func _build_player() -> void:
    var party := PARTY_SCENE.instantiate(); party.name = "Chapter08PartyRuntime"; party.position = _vec3(zone.get("entry", [0,0,0])) + Vector3.UP * 0.1; _root().add_child(party)
