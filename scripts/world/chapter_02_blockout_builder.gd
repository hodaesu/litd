extends Node3D
class_name Chapter02BlockoutBuilder
...
const CLUE_SCRIPT := preload("res://scripts/world/chapter_02_clue.gd")

@export var zone_id := "c02_old_road"
...
func build_zone() -> void:
    ...
    if zone.is_empty():
        push_error("Chapter02BlockoutBuilder: zone inconnue %s" % zone_id)
        return
...
func _root() -> Node3D:
    var root := get_node_or_null("GeneratedChapter02") as Node3D
    if root == null:
        root = Node3D.new()
        root.name = "GeneratedChapter02"
        add_child(root)
    return root
...
func _build_boundaries() -> void:
    ...
    var w := float(size[0])
    var d := float(size[1])
    _wall(Vector3(0,1,-d/2), Vector3(w,2,1))
    _wall(Vector3(0,1,d/2), Vector3(w,2,1))
    _wall(Vector3(-w/2,1,0), Vector3(1,2,d))
    _wall(Vector3(w/2,1,0), Vector3(1,2,d))
...
func _build_entry_and_exits() -> void:
    ...
    for exit_value in zone.get("exits", []):
        ...
        gate.to_zone = String(exit_data.get("to", ""))
        gate.gate_id = "%s_to_%s" % [zone_id, gate.to_zone]
        gate.position = _vec3(exit_data.get("position", [0,0,0]))
...
func _build_encounters() -> void:
    ...
    for encounter_value in zone.get("encounters", []):
        var data: Dictionary = encounter_value
        ...
        if data.has("name"):
            trigger.set_meta("miniboss", {"name":data.get("name"), "loot_tier":"major"})
        _area_box(trigger, Vector3(5,2.5,5) if trigger.encounter_type != "boss" else Vector3(8,3,8))
        parent.add_child(trigger)

func _build_clues() -> void:
    var parent := Node3D.new()
    parent.name = "InvestigationClues"
    _root().add_child(parent)
    var ids: Array = zone.get("clues", [])
    for i in ids.size():
        var clue := CLUE_SCRIPT.new() as Chapter02Clue
        clue.name = String(ids[i])
        clue.position = _clue_position(i, ids.size())
        clue.configure(String(ids[i]))
        _area_box(clue, Vector3(1.6,1.6,1.6))
...
func _build_campfire() -> void:
    if not bool(zone.get("campfire", false)):
        return
...
func _build_player() -> void:
    ...
    party.name = "Chapter02PartyRuntime"
