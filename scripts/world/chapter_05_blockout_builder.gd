extends Node3D
class_name Chapter05BlockoutBuilder
...
const FRAGMENT_SCRIPT := preload("res://scripts/world/chapter_05_fragment.gd")

@export var zone_id := "c05_black_glass_crypts"
...
func build_zone() -> void:
    ...
    if zone.is_empty(): push_error("Chapter05BlockoutBuilder: zone inconnue %s" % zone_id); return
    AshlandsRuntime.enter_zone(zone_id); _build_floor(); _build_boundaries(); _build_exits(); _build_encounters(); _build_fragments(); _build_campfire()
...
func _root() -> Node3D:
    var root := get_node_or_null("GeneratedChapter05") as Node3D
    if root == null: root = Node3D.new(); root.name = "GeneratedChapter05"; add_child(root)
    return root

func _build_floor() -> void:
    var size: Array = zone.get("size_m",[100,100]); var body := StaticBody3D.new(); _root().add_child(body)
    ...
    var col := CollisionShape3D.new(); var shape := BoxShape3D.new(); shape.size = box.size; col.shape = shape; col.position.y = -0.25; body.add_child(col)
...
func _wall(pos: Vector3, size: Vector3) -> void:
    var body := StaticBody3D.new(); body.position = pos; _root().add_child(body); var col := CollisionShape3D.new(); var shape := BoxShape3D.new(); shape.size = size; col.shape = shape; body.add_child(col)

func _build_exits() -> void:
    for value in zone.get("exits",[]):
        var d: Dictionary = value; var gate := ZoneTransitionGate.new(); gate.from_zone = zone_id; gate.to_zone = String(d.get("to","")); gate.gate_id = "%s_to_%s" % [zone_id,gate.to_zone]; gate.position = _vec3(d.get("position",[0,0,0])); _area_box(gate,Vector3(3,2.5,3)); _root().add_child(gate)

func _build_encounters() -> void:
    for value in zone.get("encounters",[]):
        var d: Dictionary = value; var trigger := EncounterTrigger.new(); trigger.encounter_id = String(d.get("id","")); trigger.encounter_type = String(d.get("type","normal")); trigger.position = _vec3(d.get("position",[0,0,0]))
...
func _build_player() -> void:
    var party := PARTY_SCENE.instantiate(); party.name = "Chapter05PartyRuntime"; party.position = _vec3(zone.get("entry",[0,0,0])) + Vector3.UP*0.1; _root().add_child(party)

func _area_box(area: Area3D, size: Vector3) -> void:
    var col := CollisionShape3D.new(); var shape := BoxShape3D.new(); shape.size = size; col.shape = shape; col.position.y = size.y*0.5; area.add_child(col)
