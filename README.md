...
`tools.qa.combat_turn_audit` verrouille le moteur de rounds à quatre héros conservé par la chaîne v6 → v5 → v4 → v3 → v2 : chaque héros vivant agit une fois par round, le compagnon agit une seule fois après le groupe, puis les ennemis. Il vérifie aussi que toutes les statistiques produites par les arbres de compétences sont réellement consommées par le combat effectif.

`tools.qa.tactical_combat_audit` vérifie la couche tactique v3 conservée sous v4/v5/v6 : quatre rangs uniques, positions d'utilisation et de ciblage, déplacement consommant l'action, techniques propres aux héros, ciblage avant/arrière des ennemis, liberté de ciblage des boss et synergies de formation.

`tools.qa.dismemberment_audit` vérifie la couche v4 conservée sous v5/v6 : jauge de trauma, profils corporels, conséquences fonctionnelles, résistance accrue des boss, absence d'exécution instantanée des boss et indépendance entre mécanique et niveau de gore affiché.
...
`tools.qa.enemy_family_tactics_audit` vérifie la couche v6 : couverture exacte des 37 ennemis génériques non-boss, familles humanoïde/bête/arachnide/aberration/construction, comportements élite et boss, priorité des mécaniques spécifiques et réactions de famille aux membres perdus.

`tools.qa.combat_economy_sim_v2` reste le modèle numérique de base pour les checkpoints de niveaux 1/10/20/30/40/50, plus de trente boss/mini-boss, les compagnons recrutés, les cycles NG+ 0→5, la vitesse d'XP et l'économie Or/Essence. Les couches tactiques v3/v4/v5/v6 ajoutent les rangs, synergies, démembrements, déplacements forcés et comportements de familles par-dessus cette base.

Les rapports sont écrits dans `reports/qa-report.json`, `reports/qa-report.html`, `reports/cross-system-report.json`, `reports/balance-report.json`, `reports/combat-turn-report.json`, `reports/tactical-combat-report.json`, `reports/dismemberment-report.json`, `reports/displacement-combat-report.json`, `reports/enemy-family-tactics-report.json` et `reports/combat-economy-report.json`.
...
- **CI** : tests Python, audits de structure/équilibrage, audit du moteur de tours, rangs/synergies, démembrements, déplacements forcés/phases de boss, familles ennemies, simulation combat-économie et smoke test Godot headless.
